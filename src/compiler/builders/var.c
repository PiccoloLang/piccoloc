...
LLVMValueRef buildVar(compiler* comp, astVar* var) {
    ...
    LLVMValueRef val;
    if(idx == -1) {
         val = buildCall(comp, comp->rtlib->evalRuntimeVariable.type, comp->rtlib->evalRuntimeVariable.func, 2, buildStrOffset(comp, comp->pkgSrcStr, buildInt(comp, var->name.start - comp->pkg->src)), buildInt(comp, var->name.len));
    } else {
        val = buildGetPtr(comp, comp->vars.vals[idx].alloc);
    }
